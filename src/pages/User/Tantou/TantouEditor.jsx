...
// Phân nhóm chapter cho tab "Duyệt Chapters", khớp đúng với cách Mangaka set
// status (xem Mangaka.jsx / ChapterView.jsx):
//   InProduction  → Mangaka/Assistant đang làm, chưa gửi Tantou
//   Ready         → Mangaka VỪA GỬI, Tantou CHƯA xử lý (không phải "đã gửi EB")
//   Delayed       → Tantou đã yêu cầu sửa, đang chờ Mangaka/Assistant làm lại
//   Published     → Tantou đã duyệt xuất bản thẳng (không qua EB)
// 'eb' chỉ dùng cho trường hợp CẢ SERIES đang ở vòng EB duyệt tổng thể (debut
// pipeline) — khác hoàn toàn với status của từng chapter.
function getChapterLayer(item) {
  ...
  if (seriesWithEb) return 'eb'
  if (st === 'ready') return 'new'
  return 'inprogress'
}
...
export default function TantouEditor() {
  ...
  const layeredStudioQueue = useMemo(() => {
    const maps = { new: new Map(), inprogress: new Map(), eb: new Map(), done: new Map() }
    ...
    const result = {}
    for (const layerKey of ['new', 'inprogress', 'eb', 'done']) {
  ...
  const studioSections = [
    { key: 'new', label: 'Mới nhận — chờ bạn duyệt', dot: 'bg-sky-500', groups: layeredStudioQueue.new },
    { key: 'inprogress', label: 'Đã yêu cầu sửa — đang chỉnh lại', dot: 'bg-amber-500', groups: layeredStudioQueue.inprogress },
    { key: 'eb', label: `Series đang chờ ${LABEL_EDITOR_BOARD} duyệt`, dot: 'bg-violet-500', groups: layeredStudioQueue.eb },
                        ...
                        <div key={section.key} className="space-y-2">
                          ...
                          {seriesChapters.map(ch => (
                            <div
                              ...
                              className={`cursor-pointer rounded-md border p-2 text-xs transition-all hover:shadow-sm ${selectedChapter.chapterid === ch.chapterid
                          ...
                          >
                            {allSeriesChapters.map(ch => (
                              <div
                                key={ch.chapterid}
                                onClick={() => openChapter(ch)}
                                className={`shrink-0 cursor-pointer rounded-lg border px-3 py-1.5 text-xs transition-all ${selectedChapter.chapterid === ch.chapterid
                ...
                <div className="space-y-4">
                  ...
                  {studioLoading ? (
                    ...
                    <>
                      ...
                      {(() => {
                        const seriesWithChapters = layeredStudioQueue.new
                          .concat(layeredStudioQueue.inprogress)
                            ...
                            >
                              {seriesWithChapters.map(group => (
                                <div
                                  ...
                                  className={`group shrink-0 cursor-pointer rounded-xl border bg-card p-3 text-xs transition-all hover:shadow-md ${selectedSeriesIdForChapters === group.seriesId
